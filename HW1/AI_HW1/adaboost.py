...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        for t in range(self.T):
            ...
            #print(featureVals, labels, features, weights.shape)
    ...
    def selectBest(self, featureVals,iis, labels, features, weight):
        """
        ...
        """
        e=[] contains error of every classifier.
        i represents different feature, j represents the index of different images.
        temp represent the error of that classifier, b represent the location.
        Therefore if j<0 means h(x)=1 and if labels[b]==0, temp+=weight*abs(1-0).
        Similarly, when j>=0 labels[b]==1.
        Finally, I use a easy method to chooose bestError, and return bestClf and bestError.
        Here, need to care for the type of bestClf.
        """
        e=[]
        for i in featureVals:
            ...
            temp=0
            for j in i:
                if j<0 and labels[b]==0:
                    temp+=weight[b]
                elif j>=0 and labels[b]==1:
                    temp+=weight[b]
                b=b+1
            e.append(temp)
        min2=e[0]
        ...
        for i in range(len(e)):
            if e[i]<min2:
                min1=i
                min2=e[i]
        bestClf=WeakClassifier(features[min1])
        bestError=min2
